...
class GeneralViewControllerz: UIViewController {

	@IBOutlet weak var generalTableViewz: UITableView!
...
//	var datas: [Read] = []
    ...
    override func viewDidLoad() {
		...
		generalTableViewz.delegate = self
		generalTableViewz.dataSource = self
		fetchGeneral()
		generalTableViewz.isHidden = true
	...
	func checkDatas() {
		if datas.isEmpty {
			...
			generalTableViewz.isHidden = false
			DispatchQueue.main.async {
				self.generalTableViewz.reloadData()
			}
...
extension GeneralViewControllerz: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if let lastVisibleIndexPath = tableView.indexPathsForVisibleRows?.last {
            if indexPath == lastVisibleIndexPath && indexPath.row == self.datas.count - 1 {
                ...
                self.generalTableViewz.tableFooterView = spinner
                self.generalTableViewz.tableFooterView?.isHidden = false
