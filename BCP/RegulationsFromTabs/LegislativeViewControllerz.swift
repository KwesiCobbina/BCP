...
class LegislativeViewControllerz: UIViewController {

	@IBOutlet weak var homeTableViewz: UITableView!
    ...
    override func viewDidLoad() {
		...
		homeTableViewz.delegate = self
		homeTableViewz.dataSource = self
		fetchLegisilative()
		homeTableViewz.isHidden = true
	...
	func checkDatas() {
		if posts.isEmpty {
			...
			homeTableViewz.isHidden = false
			DispatchQueue.main.async {
				self.homeTableViewz.reloadData()
			}
...
extension LegislativeViewControllerz: UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if let lastVisibleIndexPath = tableView.indexPathsForVisibleRows?.last {
            if indexPath == lastVisibleIndexPath && indexPath.row == self.posts.count - 1 {
                ...
                self.homeTableViewz.tableFooterView = spinner
                self.homeTableViewz.tableFooterView?.isHidden = false
...
//            let indexPaths=self.homeTableViewz!.indexPathsForSelectedRows!
//            let indexPath = indexPaths[0] as NSIndexPath
//            let destinationView = segue.destination as? RegulationDetailViewController
//            destinationView!.class_id = self.posts[indexPath.row].regulation_id
//            destinationView!.selectedRegulation = self.posts[indexPath.row]
//        }
	}
}
