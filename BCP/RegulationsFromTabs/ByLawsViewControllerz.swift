...
class ByLawsViewControllerz: UIViewController {

	@IBOutlet weak var byLawsTableViewz: UITableView!
    ...
    override func viewDidLoad() {
		...
		byLawsTableViewz.delegate = self
		byLawsTableViewz.dataSource = self
		fetchByLaws()
		byLawsTableViewz.isHidden = true
	...
	func checkDatas() {
		if datas.isEmpty {
			...
			byLawsTableViewz.isHidden = false
			DispatchQueue.main.async {
				self.byLawsTableViewz.reloadData()
			}
...
extension ByLawsViewControllerz: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if let lastVisibleIndexPath = tableView.indexPathsForVisibleRows?.last {
            if indexPath == lastVisibleIndexPath && indexPath.row == self.datas.count - 1 {
                ...
                self.byLawsTableViewz.tableFooterView = spinner
                self.byLawsTableViewz.tableFooterView?.isHidden = false
...
//			let indexPaths=self.byLawsTableViewz!.indexPathsForSelectedRows!
